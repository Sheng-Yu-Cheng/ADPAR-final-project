...
DRONE_MAX_PEAK_DB = 110.0
DRONE_MIN_TOP10_DB = 90.0
DRONE_MAX_TOP10_DB = 105.0
...
DRONE_MAX_VELOCITY_SPREAD_MPS = 0.35
...
OUT_PATH = Path(
    r"C:\Users\user\Desktop\ADPAR-final-project\adc\live_event_classifier_peakdb_v2_60s_200ms_Raw_0.bin"
)
...
def classify_event_from_db_features(
    ...
    """
    Classify EMPTY / DRONE / HUMAN using absolute heatmap dB first.

    This function uses the same dB scale that you see on the plot:
      peak_db  = max(rd_db)
      top10_db = mean(top 10 cells of rd_db)

    E/noise is still printed for debugging, but it is not the main threshold.
    """
    ...
    # Drone: medium brightness + compact RD blob.
    if in_drone_db_band and compact and not wide_or_large:
        return (
            "DRONE",
            f"compact mid-power blob: peak={peak_db:.1f}dB, top10={top10_db:.1f}dB, "
            f"bins={bins}, Rspread={r_spread:.2f}m, Vspread={v_spread:.2f}m/s, "
            f"E/noise={e_db:.1f}dB",
        )

    # Human: very bright OR wider/larger RD blob.
    if (
        ...
        return (
            "HUMAN",
            f"bright/wide blob: peak={peak_db:.1f}dB, top10={top10_db:.1f}dB, "
            f"bins={bins}, Rspread={r_spread:.2f}m, Vspread={v_spread:.2f}m/s, "
            f"E/noise={e_db:.1f}dB",
        )
...
def udp_receiver_worker():
    print("=== DCA1000 live event classifier V2: EMPTY / DRONE / HUMAN by peak/top10 dB ===")
    ...
    print("  Main thresholds use absolute heatmap dB, not E/noise.")
    print(f"  EMPTY: peak < {EMPTY_MAX_PEAK_DB:.1f} dB and top10 < {EMPTY_MAX_TOP10_DB:.1f} dB")
    print(f"  DRONE: peak {DRONE_MIN_PEAK_DB:.1f}~{DRONE_MAX_PEAK_DB:.1f} dB, compact blob")
    print(f"  HUMAN: peak >= {HUMAN_MIN_PEAK_DB:.1f} dB or top10 >= {HUMAN_MIN_TOP10_DB:.1f} dB or wide blob")
    print()
    print("IMPORTANT:")
...
def run_plot():
    ...
    ax.set_title("Event Classifier V2: EMPTY / DRONE / HUMAN by peak/top10 dB")
