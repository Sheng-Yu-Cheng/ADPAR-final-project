...
# ============================================================
# Event classification by dB features
# Classes:
#   EMPTY : no significant moving energy
#   DRONE : compact / narrow moving blob, often smaller energy and smaller range spread
#   HUMAN : larger / wider moving blob, usually stronger total energy and wider spread
#
# Important:
#   These are initial heuristic thresholds.
#   You should tune them using your real measured dB values.
# ============================================================

ENABLE_EVENT_CLASSIFICATION = True

# Main dB feature:
#   energy_db_over_noise = 10log10(component_energy / local_noise_energy)
EMPTY_MAX_DB_OVER_NOISE = 30.0

# Human tends to occupy more RD bins / range bins than Tello.
HUMAN_MIN_DB_OVER_NOISE = 60.0
HUMAN_MIN_ACTIVE_BINS = 18
HUMAN_MIN_RANGE_SPREAD_M = 0.18
HUMAN_MIN_VELOCITY_SPREAD_MPS = 0.18

# Drone is usually more compact and weaker than human body.
DRONE_MIN_DB_OVER_NOISE = 30.0
DRONE_MAX_DB_OVER_NOISE = 60.0
DRONE_MAX_ACTIVE_BINS = 22
DRONE_MAX_RANGE_SPREAD_M = 0.25
...
OUT_PATH = Path(
    r"C:\Users\user\Desktop\ADPAR-final-project\adc\live_event_classifier_db_v1_60s_200ms_Raw_0.bin"
)
...
def classify_event_from_db_features(detected_raw: bool, metrics: dict):
    """
    Heuristic dB-based event classifier.

    Returns:
      event_label, event_reason

    Uses these features:
      energy_db_over_noise
      active_bins
      range_spread_m
      velocity_spread_mps

    Tuning workflow:
      1. Run EMPTY / DRONE / HUMAN separately.
      2. Observe E/noise, bins, range spread, velocity spread.
      3. Tune thresholds above.
    """
    ...
    if not detected_raw or not np.isfinite(e_db):
        return "EMPTY", "no detected moving RD component"

    if e_db < EMPTY_MAX_DB_OVER_NOISE:
        return (
            "EMPTY",
            f"E/noise={e_db:.1f}dB < empty threshold {EMPTY_MAX_DB_OVER_NOISE:.1f}dB",
        )

    human_like = (
        e_db >= HUMAN_MIN_DB_OVER_NOISE
        or bins >= HUMAN_MIN_ACTIVE_BINS
    ...
    drone_like = (
        e_db >= DRONE_MIN_DB_OVER_NOISE
        and e_db <= DRONE_MAX_DB_OVER_NOISE
        and bins <= DRONE_MAX_ACTIVE_BINS
        and (not np.isfinite(r_spread) or r_spread <= DRONE_MAX_RANGE_SPREAD_M)
    )

    if human_like and not drone_like:
        return (
            "HUMAN",
            f"wide/strong blob: E/noise={e_db:.1f}dB, bins={bins}, "
            f"Rspread={r_spread:.2f}m, Vspread={v_spread:.2f}m/s",
        )

    if drone_like and not human_like:
        return (
            "DRONE",
            f"compact medium blob: E/noise={e_db:.1f}dB, bins={bins}, "
            f"Rspread={r_spread:.2f}m",
        )

    if human_like and drone_like:
        if bins >= HUMAN_MIN_ACTIVE_BINS or (
            np.isfinite(r_spread) and r_spread >= HUMAN_MIN_RANGE_SPREAD_M
        ):
            return (
                "HUMAN",
                f"ambiguous but wider: E/noise={e_db:.1f}dB, bins={bins}, "
                f"Rspread={r_spread:.2f}m",
            )
        return (
            "DRONE",
            f"ambiguous but compact: E/noise={e_db:.1f}dB, bins={bins}, "
            f"Rspread={r_spread:.2f}m",
        )

    return (
        "UNKNOWN_MOVING",
        f"moving but unmatched: E/noise={e_db:.1f}dB, bins={bins}, "
        f"Rspread={r_spread:.2f}m, Vspread={v_spread:.2f}m/s",
    )
...
def udp_receiver_worker():
    print("=== DCA1000 live event classifier V1: EMPTY / DRONE / HUMAN by dB ===")
    ...
    print(f"  EMPTY: E/noise < {EMPTY_MAX_DB_OVER_NOISE:.1f} dB")
    print(f"  DRONE: {DRONE_MIN_DB_OVER_NOISE:.1f}~{DRONE_MAX_DB_OVER_NOISE:.1f} dB, compact blob")
    print(f"  HUMAN: E/noise >= {HUMAN_MIN_DB_OVER_NOISE:.1f} dB or wide blob")
    print()
    print("IMPORTANT:")
...
def run_plot():
    ...
    ax.set_title("Event Classifier V1: EMPTY / DRONE / HUMAN by dB features")
