...
OUT_PATH = Path(
    r"C:\Users\user\Desktop\ADPAR-final-project\adc\live_event_classifier_peakdb_v3_drone_priority_60s_200ms_Raw_0.bin"
)
...
def classify_event_from_db_features(
    ...
    Fix for observed failure:
      Drone was detected as HUMAN because it can be bright.

    New rule:
      1. EMPTY if heatmap is weak.
      2. DRONE first if the blob is compact, even if it is bright.
      3. HUMAN only if it is bright AND wide/large, or extremely wide/large.

    This is more suitable when Tello has strong reflection but remains a compact RD blob.
    """
    ...
    # IMPORTANT: classify compact objects as DRONE before considering HUMAN.
    # This fixes the case where a close/strong drone has high dB.
    ...
    # Human should be large/wide, not merely bright.
    large_human_shape = (
    ...
    # If it is bright but still compact, keep it as DRONE/UNKNOWN rather than HUMAN.
    if bright_human_power and compact_drone_shape:
        return (
            "DRONE",
            f"bright but compact, likely drone: peak={peak_db:.1f}dB, top10={top10_db:.1f}dB, "
            f"bins={bins}, Rspread={r_spread:.2f}m, Vspread={v_spread:.2f}m/s, "
            f"E/noise={e_db:.1f}dB",
        )
    ...
    # Medium/strong but not wide. Default to DRONE if compact.
...
def udp_receiver_worker():
    print("=== DCA1000 live event classifier V3: drone-priority peak/top10 dB ===")
    ...
    print("  Drone-priority logic: compact bright blobs are DRONE before HUMAN.")
    print(f"  EMPTY: peak < {EMPTY_MAX_PEAK_DB:.1f} dB and top10 < {EMPTY_MAX_TOP10_DB:.1f} dB")
    print(f"  DRONE: compact, peak {DRONE_MIN_PEAK_DB:.1f}~{DRONE_MAX_PEAK_DB:.1f} dB")
...
def run_plot():
    ...
    ax.set_title("Event Classifier V3: drone-priority peak/top10 dB classifier")
